...
class ExhibitionModel {
    ...
    switch ($filters['sort']) {
        case 'latest': 
            ...
            break;
        case 'ending': 
            $sql .= "ORDER BY A.exhibition_end_date ASC "; 
            break;
        case 'popular': 
    ...
    foreach ($rows as $row) {
        $results[] = [
            ...
            "exhibition_start_time" => $row['exhibition_start_time'] ? date('H:i', strtotime($row['exhibition_start_time'])) : null,
            "exhibition_end_time" => $row['exhibition_end_time'] ? date('H:i', strtotime($row['exhibition_end_time'])) : null,
    ...
    "exhibition_start_time" => $exhibition['exhibition_start_time'] ? date('H:i', strtotime($row['exhibition_start_time'])) : null,
    "exhibition_end_time" => $exhibition['exhibition_end_time'] ? date('H:i', strtotime($row['exhibition_end_time'])) : null,
     ...
     *  BLOB 이미지 저장/조회
     *  ----------------------*/
    public function saveImageBlob(int $id, $stream, string $mime, string $name, int $size): void {
        $sql = "UPDATE APIServer_exhibition
                SET exhibition_image       = :blob,
                    exhibition_image_mime  = :mime,
                    exhibition_image_name  = :name,
                    exhibition_image_size  = :size,
                    update_dtm             = NOW()
                WHERE id = :id";
        $stmt = $this->pdo->prepare($sql);
        $stmt->bindParam(':blob', $stream, PDO::PARAM_LOB);
    ...
    public function getImageRow(int $id): ?array {
        $sql = "SELECT exhibition_image, exhibition_image_mime, exhibition_image_name, exhibition_image_size, update_dtm
